...
export const useLogMutation = (userId) => {
  ...
  return useMutation({
    ...
    onMutate: async (newLog) => {
      ...
      const previousDashboard = queryClient.getQueryData(['dashboard', userId, dayjs().format('YYYY-MM-DD')]);

      // Optimistically update to the new value
      queryClient.setQueryData(['dashboard', userId, dayjs().format('YYYY-MM-DD')], (old) => {
        if (!old) return old;
        const habit = old.habits.find(h => h._id === newLog.habitId);
        if (!habit) return old;

        const wasCompleted = habit.completedToday;
        const isCompletedNow = newLog.completed;
        const diff = isCompletedNow ? (wasCompleted ? 0 : 1) : (wasCompleted ? -1 : 0);

        return {
          ...old,
          summary: {
            ...old.summary,
            completedToday: Math.max(0, old.summary.completedToday + diff),
          },
    ...
    onError: (err, newLog, context) => {
      queryClient.setQueryData(['dashboard', userId, dayjs().format('YYYY-MM-DD')], context.previousDashboard);
    },
    onSettled: () => {
...
export const useHabitStatsQuery = (habitId) => {
  return useQuery({
    queryKey: ['habit-stats', habitId],
    queryFn: async () => {
      const { data } = await api.get(`/habits/${habitId}/stats`);
      return data;
    },
    enabled: !!habitId,
  });
